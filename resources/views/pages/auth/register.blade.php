<x-base-layout :$title>
    <div class="container flex h-screen items-center justify-between gap-20">
        <div class="flex flex-col justify-center gap-4">
            <h2 class="mb-4 font-medium">Daftarkan dirimu sekarang juga!</h2>
            <form action="" method="POST" class="flex flex-col items-center gap-4">
                ...
                <div class="w-full">
                    <input type="text" name="nama_lengkap" id="nama_lengkap" placeholder="Nama lengkap" value="{{ old('nama_lengkap') }}" required
                        class="w-full rounded border border-primary-950 px-4 py-2 shadow outline-none focus:ring focus:ring-primary-950 {{ $errors->has('nama_lengkap') ? '!border-red-500 focus:!ring-red-500 text-red-500 placeholder:text-red-300' : '' }}">
                        @error('nama_lengkap')
                        <p class="ml-2 mt-2 text-red-500">{{ $message }}</p>
                        @enderror
                </div>
                <div class="w-full">
                    <input type="text" inputmode="numeric" name="telp" id="telp" placeholder="No telepon" value="{{ old('telp') }}" required
                        class="w-full rounded border border-primary-950 px-4 py-2 shadow outline-none focus:ring focus:ring-primary-950 {{ $errors->has('telp') ? '!border-red-500 focus:!ring-red-500 text-red-500 placeholder:text-red-300' : '' }}">
                        @error('telp')
                        <p class="ml-2 mt-2 text-red-500">{{ $message }}</p>
                        @enderror
                </div>
            ...
            <p class="text-center">Sudah punya akun? <a href="/auth/login"
                    class="font-semibold text-primary-600 hover:opacity-80">Masuk sekarang!</a></p>
        </div>
