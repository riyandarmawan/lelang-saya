<x-dashboard.layout :$title>
    <div class="flex h-full items-center justify-center p-8">
        <div class="w-full max-w-md rounded-lg border border-primary-950 bg-white p-6 shadow-lg">
            <h3 class="mb-4 text-center font-semibold text-primary-500">Ubah Barang</h3>
            <form action="" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div x-data="{ preview: '/images/barangs/{{ $barang->gambar }}' }">
                ...
                <div>
                    ...
                    <input type="date" id="tanggal" name="tanggal" value="{{ $barang->tanggal }}" readonly type="date" id="tanggal" name="tanggal" value="{{ date('Y-m-d') }}" {{ old('gambar') }}"
                        class="@error('tanggal') border-red-500 text-red-500 placeholder:text-red-300 ring-red-500 @enderror w-full rounded-lg border px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 bg-slate-200"
                        required>
                    @error('tanggal')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" inputmode="numeric" id="harga_awal" name="harga_awal" value="{{ old('harga_awal', $barang->harga_awal) }}"
                ...
                <div>
                    ...
                    <textarea id="deskripsi_barang" name="deskripsi_barang"
                        ...
                        rows="3" required>{{ old('deskripsi_barang', $barang->deskripsi_barang) }}</textarea>
                ...
                <div>
                    ...
                    <select id="kategori_id" name="kategori_id" value="{{ old('kategori_id') }}"
                        ...
                        @foreach ($kategoris as $kategori)
                            <option {{ old('kategori_id', $barang->kategori_id) == $kategori->id ? 'selected' : '' }} value="{{ $kategori->id }}">{{ $kategori->nama_kategori }}</option>
                        @endforeach
                    </select>
                    @error('kategori_id')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
